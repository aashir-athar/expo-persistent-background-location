 ...
 *
 * The Expo `Record` bound from JS ([ExpoPersistentBackgroundLocationModule.StartConfigRecord])
 * is converted into this once, on `start`, and from then on *only* this snapshot
 * is passed around. The service and the boot receiver reload it from disk via
 * [ConfigStore] — they never touch the React bridge, so they cannot depend on
 * the Record type.
 *
