 ...
 * `src/ExpoPersistentBackgroundLocation.types.ts` exactly. The bridge payload is
 * produced by [toMap] — a `Map<String, Any?>` (never a `Bundle`) so that
 * optional numeric fields can carry an honest `null` across to JS instead of a
 * sentinel or a missing key.
 */
internal data class LocationFixModel(
  ...
  /** Bridge payload. Order is irrelevant; presence of every key is not. */
