 ...
 * outlive any React context) and the [ExpoPersistentBackgroundLocationModule]
 * (which comes and goes with the JS runtime).
 *
 * The service always publishes here; if a module is attached it forwards events
 * to JS, and if not, the fixes were already persisted to SQLite so nothing is
 * lost. The bus also holds the small `running / lastFix / isMoving` snapshot
 * that backs `getStatus()` and `isRunning()` without a round-trip to the service.
 *
