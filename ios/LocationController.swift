...
/// at which point ``resumeIfNeeded()`` re-arms the exact session.
///
...
final class LocationController: NSObject, CLLocationManagerDelegate {
  ...
  // and read from Expo's background function queue (statusDictionary / isRunning
  // / flush) and the sync-timer queue. All access goes through `stateLock` —
  // `lastFix` in particular is an ARC reference whose unsynchronized swap-while-
  // read is undefined behaviour, not just a stale read.
  ...
  /// Set by the module while a JS runtime is attached; nil when the app is
  /// running headless (post-relaunch, before JS boots).
  ...
  /// by ``ExpoPersistentBackgroundLocationAppDelegate`` from
  ...
  func flush(completion: @escaping ([String: Any?]) -> Void) {
    guard let config = config ?? ConfigStore.load(), let url = config.syncUrl, !url.isEmpty else {
      completion(SyncResultModel(success: false, count: 0, status: nil, error: "no-sync-url").toDictionary())
      return
    }
  ...
  func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
    ...
    emit(PBLConstants.eventLocation, fix.toDictionary())
    if flipped {
      emit(PBLConstants.eventMotionChange, [
        "isMoving": isMoving, "activity": activity, "fix": fix.toDictionary()
      ])
    }
  }
  ...
  private func emitProviderChange() {
    ...
    emit(PBLConstants.eventProviderChange, [
      "enabled": enabled,
      "gpsEnabled": enabled,
