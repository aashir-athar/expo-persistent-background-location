...
`expo-persistent-background-location` handles **location data**, which is
sensitive personal information (PII). We take security and privacy reports
...
1. **GitHub Security Advisories (preferred).** Go to the repository's
   **Security → Advisories → Report a vulnerability** page
   (<https://github.com/aashir-athar/expo-persistent-background-location/security/advisories/new>).
...
2. **Email.** Write to **subscriptions@hybriddot.com** with subject
   `SECURITY: expo-persistent-background-location`. If you wish to encrypt, ask in
   a first contact email and we'll arrange a key.
